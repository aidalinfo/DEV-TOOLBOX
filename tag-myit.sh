...
# Script pour créer des tags myit sur tous les microservices et le frontend
# Usage: ./tag-myit.sh [--major|--minor|--patch] [--dry-run]
...
show_help() {
  ...
  echo ""
  echo "Options:"
  echo "  --major       Incrémenter la version majeure (v1.0.0 -> v2.0.0)"
  echo "  --minor       Incrémenter la version mineure (v1.0.0 -> v1.1.0)"
  echo "  --patch       Incrémenter la version patch (v1.0.0 -> v1.0.1) [par défaut]"
  echo "  --dry-run     Afficher les tags qui seraient créés sans les créer"
  echo "  -h, --help    Afficher cette aide"
  echo ""
  echo "Exemples:"
  echo "  $0 --patch              # Crée v1.0.3-myit"
  echo "  $0 --minor              # Crée v1.1.0-myit"
  echo "  $0 --major              # Crée v2.0.0-myit"
  echo "  $0 --patch --dry-run    # Simule la création de v1.0.3-myit"
}
...
increment_version() {
  ...
  # Extraire les chiffres de la version (format: v1.0.2-myit)
  if [[ $version =~ v([0-9]+)\.([0-9]+)\.([0-9]+)-myit ]]; then
    ...
    echo "v${major}.${minor}.${patch}-myit"
  else
    echo ""
  fi
}

# Fonction pour obtenir le dernier tag myit d'un repo
get_latest_myit_tag() {
  local repo=$1
  local latest_tag=$(git tag | grep -E "^v[0-9]+\.[0-9]+\.[0-9]+-myit$" | sort -V | tail -1)
  echo "$latest_tag"
}
...
process_repo() {
  ...
  local latest_tag
  latest_tag=$(get_latest_myit_tag "$repo_path")

  if [ -z "$latest_tag" ]; then
    echo -e "${YELLOW}⚠${NC}  Aucun tag myit trouvé, utilisation du tag de base: $BASE_TAG"
    latest_tag=$BASE_TAG
  else
    echo -e "→ Dernier tag myit trouvé: ${GREEN}$latest_tag${NC}"
  fi
